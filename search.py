...
class Search():
    ...
    def load_data(self, logger):
        ...
        if self.dataset_type == "station":
            att_cnn, att_flow, att_lstm, short_cnn, short_flow, short_lstm, short_poi, y = my_data_loader.sample(datatype = "validation")
            self.val_data = [att_cnn, att_flow, att_lstm, short_cnn, short_flow, short_lstm, short_poi]
            self.val_label = y
